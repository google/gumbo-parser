...
TEST_F(GumboParserTest, BadNestedLists) {
  // generated by contenteditable in qt 4.8 webkit
  // http://www.w3.org/TR/html/grouping-content.html#the-ul-element
  Parse("<html><body><ul><li>item1</li><ul><li>item1.1</li></ul><li>item2</li></ul></body></html>");

  GumboNode* body;
  GetAndAssertBody(root_, &body);
  ASSERT_EQ(1, GetChildCount(body));

  GumboNode* list = GetChild(body, 0);
  ASSERT_EQ(GUMBO_NODE_ELEMENT, list->type);
  EXPECT_EQ(GUMBO_TAG_UL, GetTag(list));
  ASSERT_EQ(2, GetChildCount(list));

  GumboNode* list_item1 = GetChild(list, 0);
  ASSERT_EQ(GUMBO_NODE_ELEMENT, list_item1->type);
  EXPECT_EQ(GUMBO_TAG_LI, GetTag(list_item1));
  // returns 3, should return 2
  ASSERT_EQ(2, GetChildCount(list_item1));

  // GetChild(list,1) is a GUMBO_TAG_UL. the UL should be in the GetChild(GetChild(list,0),1)
  // therefore this tests also fail:
  GumboNode* list_item1_text = GetChild(list_item1, 0);
  ASSERT_EQ(GUMBO_NODE_TEXT, list_item1_text->type);
  GumboNode* list_item1_ul = GetChild(list_item1, 1);
  ASSERT_EQ(GUMBO_NODE_ELEMENT, list_item1_ul->type);
  EXPECT_EQ(GUMBO_TAG_UL, GetTag(list_item1_ul));
  ASSERT_EQ(1, GetChildCount(list_item1_ul));

  GumboNode* list_item2 = GetChild(list, 1);
  ASSERT_EQ(GUMBO_NODE_ELEMENT, list_item2->type);
  EXPECT_EQ(GUMBO_TAG_LI, GetTag(list_item2));
  ASSERT_EQ(1, GetChildCount(list_item2));
}
